...
Georgios Dimitriou\inst{2}\orcidID{0009-0006-7227-9612} \and
Maria Sierra\inst{3} \and
...
Panagiotis Trakadas\inst{2}\orcidID{0000-0002-5146-5954} \and
Luis Montesano\inst{3}
}

\institute{Institute of Informatics and Telecommunications,
  NCSR Demokritos, Ag. Paraskevi, Greece
  \email{lncs@springer.com}
  \and Four Dot Infinity, Chalandri, Greece
...
This paper presents a novel approach to anomaly detection in stream
data. Modern machine learning methods are often used for this task, by
...
often use case-specific and highly contextualized, aspects of what
is considered as an anomaly.
%
In this article we explore a novel idea that aims to involve the use
case into the training of the anomaly detection model: Instead of
aiming at a general-purpose anomaly detector, we aim at a
general-purpose way to train a case-specific anomaly detector without
direct supervision.
%
The core idea is to extract anomaly indicators from the attention
layers of a deep neural network trained on a use case-specific
task. By removing the need to train on reconstruction or prediction,
we are free to train on any supervision that is available and is
characteristic of the use case. This alleviates the most pressing
challenge of anomaly detection, namely that anomalies are vaguely
defined and thus difficult to directly supervise, while simultaneously
allowing use case-specific knowledge to be extracted from whatever
...
\keywords{AI \and Machine learning \and Transformers \and Robustness
  \and Time-Series \and anomaly detection \and noise estimation \and outlier detection}
\end{abstract}
...
second task on the same data besides sleep stage detection.
This would allow us to experiment with training
on different tasks and jointly training on multiple tasks, which we
expect to give insights about what tasks are more appropriate as
...
sequence processing and anomaly detection are ubiquitous, the
existence of simultaneous task/noise supervision is less so.
But we are confident that the exposure gained through publishing our
EEG results will create an interest in compiling and publishing such
datasets.
...
% LocalWords:  neurophysiological overfit Frouros MANOLO CNECT Zenodo
