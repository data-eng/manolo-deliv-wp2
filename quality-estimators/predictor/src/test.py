...
def plot_signals(signals, batch, outlier_threshold, dpi=1200):
    ...
    path = utils.get_path('..', '..', 'static', 'predictor', 'signals', filename=f'batch_{batch}.png')
...
def compare_bands(signals, batch, threshold, num_bands=5, dpi=1200):
    ...
    path = utils.get_path('..', '..', 'static', 'predictor', 'metrics', filename=f'batch_{batch}_bands.png')
    plt.savefig(path, dpi=dpi)
    plt.close(fig)
...
def calculate_zero_crossings(signals, batch, num_points=100, dpi=1200):
    ...
    path = utils.get_path('..', '..', 'static', 'predictor', 'metrics', filename=f'batch_{batch}_zero_crossings.png')
    plt.savefig(path, dpi=dpi)
    plt.close(fig)
...
def theil_slope_intercept(signals, batch, dpi=1200):
    ...
    path = utils.get_path('..', '..', 'static', 'predictor', 'metrics', filename=f'batch_{batch}_detrended.png')
    plt.savefig(path, dpi=dpi)
    plt.close(fig)
...
def calculate_rms(signals, batch, dpi=1200):
    ...
    path = utils.get_path('..', '..', 'static', 'predictor', 'metrics', filename=f'batch_{batch}_rms.png')
    plt.savefig(path, dpi=dpi)
    plt.close(fig)
...
def collect_metrics(signals, num_bands=5, threshold=10):
    ...
    fn = utils.get_path('..', '..', 'static', 'predictor', filename='concat_signals_metrics.json')
...
def estimate_attn_quality(dataloader, attn_matrices, cols=(['HB_1', 'HB_2'], ['time', 'seq_id', 'night'], ['majority'])):
    ...
    path = utils.get_path('..', '..', 'data', 'proc', filename=f'estim_predictor_a.csv')
    df.to_csv(path, index=False)
...
def test(data, criterion, model, visualize=False, estimate=False):
    ...
    """
    mfn = utils.get_path('..', '..', 'models', filename='predictor.pth')
